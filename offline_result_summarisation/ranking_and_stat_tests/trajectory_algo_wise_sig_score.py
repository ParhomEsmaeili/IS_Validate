    ...
    for i, app1 in enumerate(apps):
        for j, app2 in enumerate(apps):
            ...
            if test_type == 'wilcoxon':
                ...
                print(f"McNemar's test for {app1} vs {app2} on metric {metric}: p-value = {p_value}")
                print(f'both pass: {both_pass}, both fail: {both_fail}, only {app1} fails: {only_v1_fails}, only {app2} fails: {only_v2_fails}')
            else:
            ...
            # if app1 == 'nnintv1' and app2 == 'adatest4-episodefinal' and values1.dtype != bool:
            #     print(f"Comparing {app1} vs {app2} on metric {metric}: p-value = {p_value}, significant = {significant}")
...
def compute_algo_wise_better_name(output_path, df, apps, metric):
    measure = [re.search(metric_key, metric).group() for metric_key in MEASURES_OF_BETTERNESS.keys() if re.search(metric_key, metric)]
    assert len(measure) == 1, f"Could not uniquely identify test type for metric {metric}."
    measure = MEASURES_OF_BETTERNESS[measure[0]]
    ...
    for i, app1 in enumerate(apps):
        for j, app2 in enumerate(apps):
            ...
            elif measure == 'median_higher':
                if values1.median() == values2.median():
                    better = None
                else:
                    better = values1.median() > values2.median()
            elif measure == 'median_lower':
                if values1.median() == values2.median():
                    better = None
                else:
                    better = values1.median() < values2.median() #We need to use strict inequality here to 
                    #avoid ties falling into here...
...
def build_per_metric_dfs(
    ...
    for folder, folder_path in folders.items():
        metrics_storage[folder] = gather_metrics_from_folder(folder_path, metrics_config, case_name_bool=case_name_bool)
    ...
    for metric in full_list:
        ...
        if 'Case_Name' not in metrics_storage[first_algo][metric].columns:
            raise ValueError(f"'Case_Name' column not found in metric {metric} for algorithm {first_algo}. Cannot align on case names, should have a pseudo-case name for pseudotime even!")    
...
def gather_metrics_from_folder(folder_path: str, metrics_config: Dict[str, List[str]], case_name_bool) -> Dict[str, Any]:
    """
            ...
            if info == None:
                ...
                if 'cols' in info:
                    ...
                    if case_name_bool:
                        ...
                        # Insert pseudo Case_Name as first column
                        pseudo_case_name = table.index.astype(str)
                        metric_dict[metric_name] = pd.concat([
                            pd.DataFrame({'Case_Name': pseudo_case_name}, index=table.index),
                            table.loc[:, indices]
                        ], axis=1)
                    print(metric_name)
...
def parse_args():
    ...
    parser.add_argument("--case_name_bool", action='store_false', default=True, help="Whether the metric dfs have case names that we can use for alignment. If False, we will generate pseudo-case names based on the index, but this means we won't be able to align on actual case names and will just assume the order is the same across algorithms.")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(args.output_root, 'apps.json')):
        ...
        with open(os.path.join(args.output_root, 'apps.json'), 'r') as f:
            existing_apps = json.load(f)
        assert set(existing_apps) == set(args.algorithm_names), f"Existing apps in {os.path.join(args.output_root, 'apps.json')} do not match the current set of apps. Existing: {existing_apps} Current: {args.algorithm_names}. Please check the input files and ensure consistency to avoid confusion." 
    ...
    for experiment_subpath in args.experiment_subpath:
        ...
        if os.name == 'posix':
            ...
            print('==============================')
            dataset_wise_dfs[experiment_subpath.split('/')[0]] = build_per_metric_dfs(folders, experiment_subpath.split('/')[0], cfg, case_name_bool=args.case_name_bool)
        else:
