...
def validate_case(case_dir):
    ...
    # folder structure: case 
                            # - image
                                # - case_name.nii.gz
                            # - label
                                # - case_name.nii.gz
    im_dir = Path(case_dir/"image")
    label_dir = Path(case_dir/"label")
    
    im = list(im_dir.glob(f"{case_name}.nii.gz"))
    if len(im) != 1:
        return False, None, None, None
    
    # Find label file
    label_files = list(label_dir.glob(f"{case_name}.nii.gz"))
    if len(label_files) != 1:
        return False, None, None, None
    

    return True, im[0], label_files[0]
...
def process_single_case(
    ...
    assert image_array.shape == label_array.shape, f"Image and label shapes do not match for case {case_name}"
    assert image_spacing == label_spacing, f"Image and label spacings do not match for case {case_name}"
    assert image_origin == label_origin, f"Image and label origins do not match for case {case_name}"
    assert image_direction == label_direction, f"Image and label directions do not match for case {case_name}"
...
def process_parse_dataset(
    ...
    """
    Process Parse dataset with label-based instance separation.
    ...
    target_dataset_name = f"Dataset{dataset_id:03d}_Parse"
    ...
    for case_dir in all_case_dirs:
        is_valid, image_file, label_file = validate_case(case_dir)
        if is_valid:
            validated_cases.append((case_dir, image_file, label_file))
    ...
    # Create modality index mapping (CT=0 only)
    modality_index_mapping = {"CT": 0}
    ...
    if num_processes == 1:
        results = [process_single_case(case_dir, image_file, label_file,
                   ...
                   for case_dir, image_file, label_file in validated_cases]
    else:
        with multiprocessing.Pool(num_processes) as p:
            results = p.starmap_async(
                process_single_case,
                [(case_dir, image_file, label_file,
                  output_images_tr, output_labels_tr,
                  semantic_class_mapping, input_to_output_class_mapping, final_semantic_class_mapping, modality_index_mapping)
                 for case_dir, image_file, label_file in validated_cases]
            )
    ...
    for i, result in enumerate(results):
        if result:
            case_name = validated_cases[i][0].name
    ...
    dataset_json = {
        "name": "Parse2022",
        "description": "Pulmonary arterial segmentation dataset from Parse2022 challenge",
        "tensorImageSize": "3D",
...
if __name__ == "__main__":
    input_directory = "/home/parhomesmaeili/Radiology_Datasets/Parse2022/train"  # Update this path
    
    dataset_id = 31  # Adjust as needed
    num_processes = 8
